communityPGLMM <- function(formula, data = list(), family = "gaussian", sp = NULL, site = NULL, random.effects = list(), 
	REML = TRUE, s2.init = NULL, B.init = NULL, reltol = 10^-6, maxit = 500, tol.pql = 10^-6, maxit.pql = 200, 
	verbose = FALSE) {
...
communityPGLMM.binary <- function(formula, data = list(), family = "binomial", sp = NULL, site = NULL, random.effects = list(), 
	...
	plmm.binary.LL <- function(par, H, X, Zt, St, mu, nestedsp = NULL, nestedsite = NULL, REML = TRUE, verbose = FALSE) {
		...
		logdetV <- plmm.binary.logdetV(par = par, Zt = Zt, St = St, mu = mu, nestedsp = nestedsp, nestedsite = nestedsite)
		if (REML == TRUE) {
